...
function InstagramBusinessCallbackContent() {
  const searchParams = useSearchParams();
  const [closeBlocked, setCloseBlocked] = useState(false);

  useEffect(() => {
    const status = searchParams.get("status") || "";
    const source = searchParams.get("source") || "instagram_business";
    ...
    postIntegrationOAuthCompleteToOpener({
      ...
      provider: source === "meta_ads" ? "meta_ads" : "instagram_business",
      status,
...
export default function InstagramBusinessCallbackPage() {
  return (
    <Suspense>
      <InstagramBusinessCallbackContent />
    </Suspense>
  );
}
